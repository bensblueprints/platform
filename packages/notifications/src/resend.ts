...
function html(p: NotificationPayload, kind: string): string {
  const headline =
    kind === "confirm"
      ? "You're in."
      : kind === "reminder-24h"
  ...
  <p style="margin:0 0 20px">
    <a href="${p.joinUrl}" style="background:#dc2626;color:#fff;padding:12px 20px;border-radius:8px;text-decoration:none;font-weight:600">Join the session</a>
  </p>
  <p style="font-size:12px;color:#888">If the button doesn't work, paste this link: <a href="${p.joinUrl}">${p.joinUrl}</a></p>
</div>`;
}
...
export const resendAdapter: NotificationAdapter = {
  ...
  async send(sql: Sql, payload: NotificationPayload) {
    const rows = await sql<{ key: string; value: string }[]>`
      select key, value from app_settings where key in ('RESEND_API_KEY', 'RESEND_FROM')
    `;
    ...
    const res = await fetch("https://api.resend.com/emails", {
      ...
      body: JSON.stringify({
        ...
        to: payload.email,
        subject,
        html: html(payload, payload.kind),
      }),
    });
