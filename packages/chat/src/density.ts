...
export function densityBandOk(actual: number, target: number): boolean {
  ...
  // the 15% rule dominates unchanged. Ceiling so a 43-target doesn't fail
  // a 50-line script by half a line.
  return Math.abs(actual - target) <= Math.max(Math.ceil(target * 0.15), 2);
}
