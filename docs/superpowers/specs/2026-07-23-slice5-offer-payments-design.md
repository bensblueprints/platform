...
| Live price tick | **CHANGED at build time:** spec named Supabase Realtime. On this box, Kong redirects every Supabase API route to `/login` (verified 2026-07-23: `/rest/v1/`, `/auth/v1/health`, `/realtime/v1/websocket` all 302 regardless of auth), so browser→Realtime is not viable. Shipped instead: SSE endpoint `/api/offers/[id]/stream` (2s row watch, push on change); client recomputes the ladder exactly as it would on a Realtime event. Migration 0006 still publishes `offers` on `supabase_realtime` so the spec's transport can be restored if Kong is fixed. Anon SELECT policy on `offers` kept (harmless, useful then). | §9 + §15 "price rise pushes live to a second open browser" |
